...
it('writes config to the specified file', function () {
    ...
    expect($content)->toHaveKey('bugsnag')
        ->and($content['bugsnag']['command'])->toBe('php')
        ->and($content['bugsnag']['args'])->toContain('mcp:start');
...
it('merges with existing mcp json content when writing', function () {
    $file = tempnam(sys_get_temp_dir(), 'mcp').'.json';
    file_put_contents($file, json_encode(['other-server' => ['command' => 'node']]));
    ...
    expect($content)->toHaveKey('bugsnag')
        ->and($content)->toHaveKey('other-server');
